// Command icondump renders every item-inventory icon in a rev-254 game cache to
// a 32x32 PNG, headlessly (no window, no GPU). It drives the same client render
// path the running game uses for the inventory/sidebar — config + textures +
// deterministic HSL palette + per-id model blobs → ObjType.GetSprite → Pix32 —
// and is the end-to-end gate for the item-icon rasterizer port: its output is
// diffed byte-for-byte against the pinned LostCityRS/Client-Java goldens
// (commit 2e629784; see cmd/icondump/icondump_test.go and tools/iconref/java/).
//
// Usage:
//
...
// legitimate skip. Java 254 arg order: getSprite(outlineRgb, count, id).
func renderSprite(id int, verbose bool) (spr *pix32.Pix32) {
	...
	return objtype.GetSprite(0, 1, id)
}
