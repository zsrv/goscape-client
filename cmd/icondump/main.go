// Command icondump renders every item-inventory icon in a rev-274 game cache to
...
// diffed byte-for-byte against the vendored LostCityRS/Client-TS goldens
// (see cmd/icondump/icondump_test.go and tools/iconref/).
//
// Usage:
//
...
func run(cfg iconConfig) (stats, error) {
	...
	objtype.Init(jio.NewJagFile(configRaw))
	...
	// (Client-TS Pix3D.lowMem default), selecting the 128x128 texel path.
