...
// referenceJagDirEnv names an env var pointing at a directory holding the three
// client jag archives (config, models, textures). TestIcondumpMatchesGoldens
// skips with a clear message when it is unset. (225 loads jag files directly —
// there is no classic .dat/.idx cache, so the env points at a jag dir, not a
// cache dir.)
const referenceJagDirEnv = "ICONDUMP_TEST_JAGDIR"

// sampleEntry mirrors one record of testdata/icons225/sample.json: the obj id
...
// against the reference jag dir into a fresh temp dir, then byte-compares every
...
// render code, commit cc3781de). A match proves the Go item-icon path reproduces
// the client pixels.
//
...
func TestIcondumpMatchesGoldens(t *testing.T) {
	...
	if jagDir == "" {
		t.Skipf("%s unset; set it to a dir holding the config, models and textures jag archives", referenceJagDirEnv)
	}

	samples := readSamples(t)

	out := t.TempDir()
	st, err := run(iconConfig{jagDir: jagDir, outDir: out, id: -1})
	if err != nil {
		t.Fatalf("run: %v", err)
	}
	if st.rendered == 0 {
	...
	for _, s := range samples {
		t.Run(s.Debugname, func(t *testing.T) {
			...
			want := readGolden(t, filepath.Join("testdata", "icons225", s.Debugname+".rgba"))
			if len(want) != 32*32*4 {
...
func readSamples(t *testing.T) []sampleEntry {
	t.Helper()
	raw, err := os.ReadFile(filepath.Join("testdata", "icons225", "sample.json"))
	if err != nil {
