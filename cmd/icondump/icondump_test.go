...
// sampleEntry mirrors one record of testdata/icons254/sample.json: the obj id
...
// render code, commit 2e629784). A match proves the Go item-icon path reproduces
// the client pixels.
//
...
func TestIcondumpMatchesGoldens(t *testing.T) {
	...
	for _, s := range samples {
		t.Run(s.Debugname, func(t *testing.T) {
			...
			want := readGolden(t, filepath.Join("testdata", "icons254", s.Debugname+".rgba"))
			if len(want) != 32*32*4 {
...
func readSamples(t *testing.T) []sampleEntry {
	t.Helper()
	raw, err := os.ReadFile(filepath.Join("testdata", "icons254", "sample.json"))
	if err != nil {
