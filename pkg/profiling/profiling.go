...
// triggered by SIGUSR1 on Unix systems. See
// docs/superpowers/specs/2026-05-22-perf-profiling-design.md for the
// full design.
//
// The signal-triggered entry point Start lives in two build-tagged
// files: profiling_signal.go (//go:build unix) registers the SIGUSR1
// handler, while profiling_stub.go (//go:build !unix) provides a no-op
// for targets without POSIX signals — notably js/wasm and Windows,
// where syscall.SIGUSR1 does not exist. Everything in this file is
// platform-neutral.
package profiling
