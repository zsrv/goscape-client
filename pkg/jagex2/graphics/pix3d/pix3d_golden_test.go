...
// LostCityRS/Client-Java render code (commit 176a85f7). Two artifacts under
// cmd/icondump/testdata:
//
...
// initColourTable's Math.random jitter term out so 0.8 is used verbatim). The Go
...
//
//	Java: Pix2D.bind(px, W, H); Pix3D.init2D();
//	    Pix3D.lowDetail = c.lowDetail; Pix3D.hclip = c.hclip; Pix3D.trans = c.trans
//
// Go equivalents:
//   - pix2d.SetPixels(width, data, height)  == Java Pix2D.bind (data, width, height)
//   - pix3d.Init2D()                        == Java Pix3D.init2D() (setRenderClipping)
//     (builds LineOffset and sets CenterW3D/CenterH3D = W/2, H/2 = 16 for 32x32,
//     which the textured raster reads as the screen origin)
//   - LowDetail / HClip / Trans package vars == Java lowDetail / hclip / trans
...
// the Go/Java parameter order. The manifest names its args screen-coord
// x-major-first (xA,xB,xC,yA,yB,yC,...), matching the vendored TS naming that
// dump.ts established; the Go port mirrors JAVA, which lists them y-major-first
// (yA,yB,yC,xA,xB,xC,..) and groups the textured view-space coords AXIS-major.
// Mappings verified against the Java Pix3D signatures at 176a85f7 and the Go
// signatures in pix3d.go:
//
//	gouraud  Java gouraudTriangle(xA,xB,xC, yA,yB,yC, cA,cB,cC)
//	         Go   GouraudTriangle(yA,yB,yC, xA,xB,xC, cA,cB,cC)
//	flat     Java flatTriangle(xA,xB,xC, yA,yB,yC, colour)
//	         Go   FlatTriangle(yA,yB,yC, xA,xB,xC, colour)
//	texture  Java textureTriangle(xA,xB,xC, yA,yB,yC, sA,sB,sC,
...
// unset.
//
// The reference ran with lowMem = false (Java Pix3D.lowMem default), which selects
// getTexels' 128x128 / 64->128-upsampling path and a 65536-entry texel pool, so
// we force LowMem = false here to match the goldens. Order mirrors IconDump.java:
...
func ensureTextures(t *testing.T, ready *bool) {
	...
	Reset()
	LowMem = false // match the reference (Java Pix3D.lowMem default = false)
	UnpackTextures(jag)
	InitColourTableDeterministic(goldenBrightness)
	InitPool(20)
	*ready = true
