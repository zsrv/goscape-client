...
// reference goldens produced by tools/iconref/dump.ts from the VENDORED
// LostCityRS/Client-TS render code. Two artifacts under cmd/icondump/testdata:
//
...
// referenceCacheEnv names the env var pointing at a classic .dat/.idxN cache dir
// (main_file_cache.dat + main_file_cache.idx0..4). Texture-dependent cases skip
// with a clear message when it is unset.
const referenceCacheEnv = "ICONDUMP_TEST_CACHE"

// The reference pins brightness = 0.8 with zero jitter (dump.ts pins
// Math.random => 0.5 so 0.5*0.03-0.015 == 0). The Go deterministic hook must
// reproduce the palette exactly at this brightness.
...
// int32 exactly. No cache is needed: the HSL colour table does not depend on
// any loaded texture.
...
// cache; they skip (via ensureTextures) when ICONDUMP_TEST_CACHE is unset.
//
...
func TestTriangleGoldens(t *testing.T) {
	...
	// Deterministic palette for the gouraud/flat cases; no cache required.
	Reset()
	InitColourTableDeterministic(goldenBrightness)
...
// dump.ts sets before each draw:
//
//	TS: Pix2D.setPixels(px, W, H); Pix3D.setRenderClipping();
//	    Pix3D.lowDetail = c.lowDetail; Pix3D.hclip = c.hclip; Pix3D.trans = c.trans
//
// Go equivalents:
//   - pix2d.SetPixels(width, data, height)  == TS Pix2D.setPixels(px, W, H)
//   - pix3d.Init()                          == TS Pix3D.setRenderClipping()
//     (builds LineOffset and sets CenterW3D/CenterH3D = W/2, H/2 = 16 for 32x32,
//     which the textured raster reads as the screen origin)
//   - LowDetail / HClip / Trans package vars == TS lowDetail / hclip / trans
...
// the Go/Java parameter order. The TS->Go arg order is NOT 1:1: the vendored TS
// signatures list the screen coords x-major-first (xA,xB,xC,yA,yB,yC,...) while
// the Go port mirrors JAVA, which lists them y-major-first (yA,yB,yC,xA,xB,xC,..)
// and groups the textured view-space coords AXIS-major. Mappings verified against
// tools/iconref/vendor/dash3d/Pix3D.ts and the Go signatures in pix3d.go:
//
//	gouraud  TS gouraudTriangle(xA,xB,xC, yA,yB,yC, cA,cB,cC)
//	         Go GouraudTriangle(yA,yB,yC, xA,xB,xC, cA,cB,cC)
//	flat     TS flatTriangle(xA,xB,xC, yA,yB,yC, colour)
//	         Go FlatTriangle(yA,yB,yC, xA,xB,xC, colour)
//	texture  TS textureTriangle(xA,xB,xC, yA,yB,yC, sA,sB,sC,
//	                            originX,originY,originZ, txB,txC, tyB,tyC, tzB,tzC, texture)
//	         Go TextureTriangle(yA,yB,yC, xA,xB,xC, sA,sB,sC,
//	                            originX,txB,txC, originY,tyB,tyC, originZ,tzB,tzC, texture)
...
// ensureTextures loads the textures jag (archive 0, file 6) from the reference
// cache and rebuilds the deterministic palette + texel pool the way the icon
// path does, exactly once. It skips the whole texture path when the env var is
// unset.
//
// The reference ran with lowMem = false (TS Pix3D.lowMem default), which selects
// getTexels' 128x128 / 64->128-upsampling path and a 65536-entry texel pool, so
// we force LowMem = false here to match the goldens. Order mirrors dump.ts:
// unpackTextures -> initColourTable(0.8) -> initPool(20).
func ensureTextures(t *testing.T, ready *bool) {
	t.Helper()
	cacheDir := os.Getenv(referenceCacheEnv)
	if cacheDir == "" {
		t.Skipf("%s unset; skipping texture-dependent golden (set it to a dir with main_file_cache.dat + main_file_cache.idx0..4)", referenceCacheEnv)
	}
	if *ready {
		return
	}

	jag := loadTexturesJag(t, cacheDir)

	Reset()
	LowMem = false // match the reference (TS Pix3D.lowMem default = false)
	UnpackTextures(jag)
	InitColourTableDeterministic(goldenBrightness)
	InitPool(20)
	*ready = true
...
// loadTexturesJag reads archive 0, file 6 (the textures jag) via the repo's own
// FileStream. The classic cache stores archive 0 in main_file_cache.idx0, and
// each dat sector tags it with the 1-based archive id (1) — the same convention
// NewFileStreamCache uses (idx[i] -> archive tag i+1). Archive 0 is stored raw
// (not gzip-compressed), so the bytes feed straight into NewJagFile.
func loadTexturesJag(t *testing.T, cacheDir string) *io.JagFile {
	t.Helper()
	dat, err := os.Open(filepath.Join(cacheDir, "main_file_cache.dat"))
	if err != nil {
		t.Fatalf("open dat: %v", err)
	}
	t.Cleanup(func() { _ = dat.Close() })
	idx0, err := os.Open(filepath.Join(cacheDir, "main_file_cache.idx0"))
	if err != nil {
		t.Fatalf("open idx0: %v", err)
	}
	t.Cleanup(func() { _ = idx0.Close() })

	fs := io.NewFileStream(dat, idx0, 1, 500000)
	raw := fs.ReadFromFile(6)
	if raw == nil {
		t.Fatalf("textures jag (archive 0, file 6) not found in %s", cacheDir)
	}
