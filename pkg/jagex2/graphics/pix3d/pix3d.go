...
// SetBrightness builds the HSL→RGB colour table and the gamma-corrected texture
// palettes. Java: Pix3D.initColourTable (Pix3D.java:260 @01f16088; the Go name
// SetBrightness is pre-244 legacy). The reference client jitters the brightness
// by a random term (rand.Float64()*0.03-0.015, mirroring the Java Math.random()
// jitter) so each run varies slightly; this faithful entry point preserves that.
// The shared, jitter-free body lives in initColourTable.
func SetBrightness(arg1 float64) {
	initColourTable(arg1 + (rand.Float64()*0.03 - 0.015))
}

// InitColourTableDeterministic builds the same colour table and texture palettes
// as SetBrightness but with NO brightness jitter: the given brightness is used
// verbatim, making the result bit-reproducible. The golden palette.bin pins
// brightness = 0.8 exactly (the reference harness forces its Math.random() jitter
// to zero to obtain the same value — see tools/iconref/java/).
//
// EXTENSION: not present in the Java client — added so the item-icon rasterizer's
// palette and triangle goldens can be pinned deterministically.
func InitColourTableDeterministic(brightness float64) {
	initColourTable(brightness)
}

// initColourTable is the shared body of SetBrightness /
// InitColourTableDeterministic. brightness is the already-resolved gamma exponent
// (jittered by the faithful caller, exact for the deterministic one). Java 244's
// initColourTable has NO transparent-collapse guard on the texture palette (that
// guard is a 274-only addition — Pix3D.java:340-342 @32f30626 — deliberately not
// ported here to stay faithful to the 01f16088 pin).
...
func TextureTriangle(arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13, arg14, arg15, arg16, arg17, arg18 int) {
	...
	// Java: Pix3D.java:1452-1460 (pin 01f16088) — the u/v/w gradient
	// cross-products and their per-scanline accumulation are 32-bit int
	// arithmetic in Java (wrap mod 2^32). Typed int32 here so Go reproduces the
	// wrap; sign-extended back to int at the textureRaster hand-off (audit
	// pix3d-2-01, backport for the F2 overflow golden: the uStride var21<<8 =
	// 2_457_600_000 exceeds int32 max and Java folds it to a negative int32).
...
func TextureRaster(arg0 []int, arg1 []int, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12, arg13, arg14 int) {
	...
	if LowMem {
		...
		// Java: Pix3D.java:2262-2264 (pin 01f16088) — perspective numerators are
		// 32-bit int products that wrap mod 2^32 (audit pix3d-C-01). Masked by the
		// arg2 clamp on the F2 overflow golden, but wrapped for Java parity with
		// rev-254/274; the following P6 sums depend on the wrapped value.
		var32 = int(int32(arg9 + (arg12>>3)*var20))
		var33 = int(int32(arg10 + (arg13>>3)*var20))
		var34 = int(int32(arg11 + (arg14>>3)*var20))
		var19 = var34 >> 12
		if var19 != 0 {
			arg2 = var32 / var19
			arg3 = var33 / var19
		...
		// Java: Pix3D.java:2275-2277 (pin 01f16088) — int sums wrap mod 2^32
		// before the >>12 divisor (backport of rev-274 audit P6 family).
		...
		if Opaque {
			for ; var16 > 0; var16-- {
				...
				arg2 = var17
				arg3 = var18
				// Java: Pix3D.java:2318-2320 (pin 01f16088) — int += wraps
				// mod 2^32 before the >>12 divisor (backport of rev-274 audit P6).
			...
			for ; var16 > 0; var16-- {
				...
				arg2 = var17
				arg3 = var18
				// Java: Pix3D.java:2402-2404 (pin 01f16088) — int += wraps
				// mod 2^32 before the >>12 divisor (backport of rev-274 audit P6).
				arg9 = int(int32(arg9 + arg12))
				arg10 = int(int32(arg10 + arg13))
				arg11 = int(int32(arg11 + arg14))
	...
	var18 = 0
	var20 = arg5 - CenterW3D
	// Java: Pix3D.java:2088-2090 (pin 01f16088) — perspective numerators are
	// 32-bit int products that wrap mod 2^32 (audit pix3d-C-01). Masked by the
	// arg2 clamp on the F2 overflow golden, but wrapped for Java parity with
	// rev-254/274; the following P6 sums depend on the wrapped value.
	var32 = int(int32(arg9 + (arg12>>3)*var20))
	...
	// Java: Pix3D.java:2101-2103 (pin 01f16088) — int sums wrap mod 2^32
	// before the >>14 divisor (backport of rev-274 audit P6 family).
	arg9 = int(int32(var32 + arg12))
	arg10 = int(int32(var33 + arg13))
	arg11 = int(int32(var34 + arg14))
	var19 = arg11 >> 14
	...
	if Opaque {
		for ; var16 > 0; var16-- {
			...
			arg2 = var17
			arg3 = var18
			// Java: Pix3D.java:2144-2146 (pin 01f16088) — int += wraps
			// mod 2^32 before the >>14 divisor (backport of rev-274 audit P6).
			arg9 = int(int32(arg9 + arg12))
			arg10 = int(int32(arg10 + arg13))
			arg11 = int(int32(arg11 + arg14))
			var19 = arg11 >> 14
	...
	for ; var16 > 0; var16-- {
		...
		arg2 = var17
		arg3 = var18
		// Java: Pix3D.java:2227-2229 (pin 01f16088) — int += wraps
		// mod 2^32 before the >>14 divisor (backport of rev-274 audit P6).
		arg9 = int(int32(arg9 + arg12))
		arg10 = int(int32(arg10 + arg13))
		arg11 = int(int32(arg11 + arg14))
