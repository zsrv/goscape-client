# iconref/java — Java-245.2 reference harness for the item-icon rasterizer
...
It is the rev-245.2 analogue of goscape-client rev-274's TypeScript harness
...
- Pinned client: **LostCityRS/Client-Java**, commit **`176a85f7`** (the
  Server245.2-ref javaclient pin). Nothing in the client is vendored here; the
  build checks out that commit into a throwaway git worktree.
- Reference cache (read-only): `LostCityRS/Server245.2-ref/unpack-ref/cache/`
  (`main_file_cache.dat` + `idx0..idx4`).
- debugname↔id map: `LostCityRS/Server245.2-ref/content/pack/obj.pack`
  (2978 entries; `ObjType.count` decodes to 2978, matching).
...
## Client-API deltas vs rev-254's harness (176a85f7 vs 2e629784)

The 245.2 pin is one lineage step earlier than 254; the icon closure is
logic-identical but several client symbols were renamed / re-ordered later, so
this harness calls them by their 245.2 names:

| use | rev-254 (2e629784) | rev-245.2 (176a85f7) |
|-----|--------------------|----------------------|
| archive class | `JagFile` | `Jagfile` |
| 2D clip init | `Pix3D.init()` | `Pix3D.init2D()` |
| bind target | `Pix2D.setPixels(W, px, H)` | `Pix2D.bind(px, W, H)` *(data, width, height)* |
...
`model.Unpack`, `Pix32.Pixels`, `io.Jagfile`) already uses its own 245.2 names;
...
- `tri/*.rgba` + `tri/manifest.json` — 8 synthetic single-triangle goldens
  ...
  1:1 into both.
- `icons2452/*.rgba` + `*.png` + `sample.json` — the curated item icons rendered
  via `ObjType.getIcon(0, 1, id)` (245.2 order: `outlineRgb, count, id`).
  (`icons2452` = rev-245.2; the `.` is dropped from the directory name.)

## 245.2-vs-254/274 golden notes
...
- `palette.bin` and **all 8 tri goldens** — gouraud, flat **and the three
  ...
  245.2's `initColourTable` has **no** transparent-collapse guard on the texture
  palette (`if ((texPal & 0xF8F8FF) == 0 && j != 0) texPal = 1;` is a 274-only
  addition at Pix3D.java:340-342 @32f30626, absent at both 176a85f7 and
...
- **P6 int32-wrap gap (latent, not reached):** rev-245.2's Go `GouraudRaster` /
  `TextureRaster` / `TextureTriangle` use plain 64-bit `int` arithmetic, whereas
  the Java-245.2 raster is 32-bit (wraps mod 2³²). The explicit `int32()` wrap
  fidelity fixes rev-254/274 carry are *later-rev additions* and are deliberately
  **not** back-ported here (no-forward-port policy — faithful to 176a85f7). No
  sample icon and no tri case reaches an int32-overflowing numerator, so every
  golden renders identically to the 32-bit reference (full run 2978/2978 = 100%,
...
## Sample composition (`icons2452/sample.json`)

Same algorithm as dump.ts: seed the two mandatory items `bronze_dagger` (245.2 id
1205) and `knife` (946), then sweep the obj space filling each category bucket to
5. Of the 9 categories, **two have no members in the 245.2 obj space** —
`resized` (no obj sets a non-128 resize) and `ambient_contrast` (its carriers, the
eadgar/viking quest items, are post-245.2 additions) — so the sample lands at 34
